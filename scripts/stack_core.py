...
class TraceConverter:
  ...
  def MatchStackRecords(self):
        ...
        for fp, tag, i in values[addr]:
          ...
          if tag + tag_offset == fault_tag:
            if obj_offset >= 0 and obj_offset < size:
              ...
              distance = -obj_offset
              whence = BEFORE
            # We prefer the closest, and if multiple objects match the most recent one.
            matching_objs.append((distance, whence, i, obj_offset, local_name, function_name, file_line))

    if not matching_objs:
      return

    matching_objs.sort()
    distance, whence, _, obj_offset, local_name, function_name, file_line = matching_objs[0]
    if whence == INSIDE:
      distance = obj_offset
      whence_str = "inside"
    elif whence == AFTER:
    ...
    print('  %d bytes %s a variable "%s" in stack frame of function "%s"'% (distance, whence_str, local_name, function_name))
