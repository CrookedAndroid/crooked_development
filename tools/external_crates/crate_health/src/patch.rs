    ...
    pub fn parse(contents: &'a str) -> Self {
        ...
        if header_and_stat.trim().is_empty() {
            ...
            let header = match header_and_stat.rfind("\n---\n") {
                Some(stat_sep) => &header_and_stat[..stat_sep + STAT_SEP.len()],
...
mod tests {
    ...
    #[test]
    fn test_mls_rs_core() {
        let contents = include_str!("testdata/mlrs_rs_core.patch");
        let parsed = Patch::parse(contents);
        assert_eq!(parsed.header, include_str!("testdata/mlrs_rs_core.header"));
