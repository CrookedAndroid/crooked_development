...
impl TestMappingJson {
    pub fn parse(mut contents: String) -> Result<TestMappingJson> {
        ...
        json_strip_comments::strip(contents)?;
...
"###;
    ...
    #[test]
    fn add_new_tests_to_postsubmit() -> Result<()> {
        let mut json = TestMappingJson::parse(TEST_JSON.to_string())?;
