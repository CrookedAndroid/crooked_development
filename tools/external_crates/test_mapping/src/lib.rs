...
impl TestMapping {
    ...
    pub fn read(path: RootedPath) -> Result<TestMapping> {
        let bpfile = path.join("Android.bp").unwrap();
        if !bpfile.abs().exists() {
            return Err(anyhow!("Blueprint file {} does not found", bpfile));
        }
        let bp = BluePrint::from_file(bpfile).map_err(|e: String| anyhow!("{}", e))?;
    ...
    pub fn update_presubmits(&mut self) -> Result<()> {
    ...
    pub fn update_imports(&mut self) -> Result<()> {
    ...
    fn libs(&self) -> Result<Vec<String>> {
        ...
        for module in &self.bp.modules {
            if matches!(
                ...
                libs.push(
                    module
                        .get_string("name")
                        .ok_or(anyhow!("rust_library rule has no name"))?
                        .clone(),
                );
            }
        }
