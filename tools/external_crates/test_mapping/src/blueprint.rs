...
pub(crate) trait RustTests {
    ...
    fn rust_tests(&self) -> Result<BTreeSet<String>>;
}
    ...
    fn rust_tests(&self) -> Result<BTreeSet<String>> {
        ...
        for module in &self.modules {
            if matches!(module.typ.as_str(), "rust_test" | "rust_test_host") {
                let name = module
                    .get_string("name")
                    .ok_or(anyhow!("{} rule has no name", module.typ))?
                    .clone();
...
pub(crate) trait RustDeps {
    ...
    fn rust_deps(&self) -> Result<BTreeSet<String>>;
}
        ...
        if let Value::ConcatExpr(expr) = self {
            for term in expr {
                match term {
                    ...
                    _ => {
                        println!("Concat term is neither ident not array");
                    }
                }
            }
        } else {
