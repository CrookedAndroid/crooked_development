...
fn get_spdx_expr(crate_name: &str, cargo_toml_license: Option<&str>) -> Result<String> {
    // Check special cases.
    if let Some((raw, expr)) = LICENSE_EXPR_SPECIAL_CASES.get(crate_name) {
        if *raw != cargo_toml_license {
            return Err(anyhow!("Found a license expression special case for crate {} but the license doesn't match. Expected {:?}, found {:?}", crate_name, raw, cargo_toml_license));
        }
    ...
    if let Some(lic) = cargo_toml_license {
        ...
        Err(anyhow!("Crate {} doesn't have a license field in Cargo.toml, and no special case was found for this crate", crate_name))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_get_chosen_licenses() -> Result<()> {
        assert_eq!(
