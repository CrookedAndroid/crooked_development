...
void ABIWrapper::SetupAvailabilityAttrs(
    repr::HasAvailabilityAttrs *decl_ir,
    const clang::Decl *decl) {
  for (const clang::AvailabilityAttr *attr : decl->specific_attrs<clang::AvailabilityAttr>()) {
    if (attr->getPlatform()->getName() != ast_contextp_->getTargetInfo().getPlatformName()) {
      continue;
    }
