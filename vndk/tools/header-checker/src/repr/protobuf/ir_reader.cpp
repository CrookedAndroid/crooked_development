...
ReadAvailabilityAttrs(record_type_protobuf.availability_attrs(), &record_type_ir);
...
void ProtobufIRReader::ReadGlobalVariables(
  ...
  for (auto &&global_variable_protobuf : tu.global_vars()) {
    ...
    ReadAvailabilityAttrs(global_variable_protobuf.availability_attrs(), &global_variable_ir);
