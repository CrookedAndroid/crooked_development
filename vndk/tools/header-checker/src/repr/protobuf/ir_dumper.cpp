...
void AddAvailabilityAttrs(HasAvailabilityAttrsMessage *decl_protobuf,
  ...
  for (const AvailabilityAttrIR &attr : decl_ir->GetAvailabilityAttrs()) {
    abi_dump::AvailabilityAttr *attr_protobuf = decl_protobuf->add_availability_attrs();
    if (attr.IsIntroduced()) {
      attr_protobuf->set_introduced_major(attr.GetIntroduced());
    }
    if (attr.IsDeprecated()) {
      attr_protobuf->set_deprecated_major(attr.GetDeprecated());
    }
    if (attr.IsObsoleted()) {
      attr_protobuf->set_obsoleted_major(attr.GetObsoleted());
    }
