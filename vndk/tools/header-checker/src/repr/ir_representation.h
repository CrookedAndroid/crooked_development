...
class AvailabilityAttrIR {
 public:
  ...
  uint32_t GetIntroduced() const { return introduced_major_; }

  bool IsIntroduced() const { return introduced_; }
  ...
  uint32_t GetDeprecated() const { return deprecated_major_; }
  ...
  uint32_t GetObsoleted() const { return obsoleted_major_; }

  bool IsObsoleted() const { return obsoleted_; }
...
class ModuleIR {
 ...
 private:
  ...
  std::optional<utils::ApiLevel> availability_;
};
