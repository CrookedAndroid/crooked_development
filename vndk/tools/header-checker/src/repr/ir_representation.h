...
class AvailabilityAttrIR {
 public:
  void SetIntroduced(uint32_t version) {
    introduced_major_ = version;
    introduced_ = true;
  }

  uint32_t GetIntroduced() const {
    return introduced_major_;
  }
  ...
  void SetDeprecated(uint32_t version) {
    deprecated_major_ = version;
    deprecated_ = true;
  }

  uint32_t GetDeprecated() const {
    return deprecated_major_;
  }
  ...
  void SetObsoleted(uint32_t version) {
    obsoleted_major_ = version;
    obsoleted_ = true;
  }

  uint32_t GetObsoleted() const {
    return obsoleted_major_;
  }
 ...
 private:
  uint32_t introduced_major_ = 0;
  uint32_t deprecated_major_ = 0;
  uint32_t obsoleted_major_ = 0;
  bool introduced_ = false;
  bool deprecated_ = false;
  bool obsoleted_ = false;
  bool unavailable_ = false;
};

//using AvailabilityAttrs = ;
...
class RecordTypeIR : public TypeIR, public TemplatedArtifactIR, public HasAvailabilityAttrs {
 public:
  enum RecordKind {
    struct_kind,
    class_kind,
...
class GlobalVarIR : public LinkableMessageIR, public ReferencesOtherType, public HasAvailabilityAttrs {
 public:
  // Add Methods to get information from the IR.
  void SetName(std::string &&name) {
    name_ = std::move(name);
...
class FunctionIR : public LinkableMessageIR, public TemplatedArtifactIR,
                   public CFunctionLikeIR, public HasAvailabilityAttrs {
 public:
  void SetAccess(AccessSpecifierIR access) {
    access_ = access;
  }
