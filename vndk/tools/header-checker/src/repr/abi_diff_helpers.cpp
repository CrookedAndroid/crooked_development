...
DiffStatus AbiDiffHelper::FilterOutRenamedRecordFields(
  ...
  // If this function returns zero, the fields may be compatible.
  // If it returns non-zero, the fields must be incompatible.
  auto compare_ignore_type = [](const RecordFieldIR *first,
                                const RecordFieldIR *second) {
    if (first->GetOffset() != second->GetOffset()) {
      return first->GetOffset() < second->GetOffset() ? -1 : 1;
    }
    if (first->IsBitField() != second->IsBitField()) {
      return first->IsBitField() < second->IsBitField() ? -1 : 1;
    }
    if (first->GetBitWidth() != second->GetBitWidth()) {
      return first->GetBitWidth() < second->GetBitWidth() ? -1 : 1;
    }
    // Skip GetReferencedType because the same type in old and new dumps may
    // have different IDs, especially in the cases of anonymous types and
    // multiple definitions.
    return 0;
  };
  // Sort fields by (offset, bit width, type).
  auto is_less = [&compare_ignore_type](const RecordFieldIR *first,
                                        const RecordFieldIR *second) {
    int result = compare_ignore_type(first, second);
    return result != 0
               ? result < 0
  ...
  while (old_it != old_end && new_it != new_end) {
    ...
    if (compare_ignore_type(*old_it, *new_it) < 0 || next_old_it - old_it > 1) {
    ...
    if (compare_ignore_type(*old_it, *new_it) > 0 || next_new_it - new_it > 1) {
