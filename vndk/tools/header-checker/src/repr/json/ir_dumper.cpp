...
static void AddAvailabilityAttrs(JsonObject &decl,
  ...
  for (auto &&attr_ir : decl_ir->GetAvailabilityAttrs()) {
    JsonObject attr;
    if (attr_ir.IsIntroduced()) {
      attr.Set("introduced_major", (uint64_t)attr_ir.GetIntroduced());
    }
    if (attr_ir.IsDeprecated()) {
      attr.Set("deprecated_major", (uint64_t)attr_ir.GetDeprecated());
    }
    if (attr_ir.IsObsoleted()) {
      attr.Set("obsoleted_major", (uint64_t)attr_ir.GetObsoleted());
    }
