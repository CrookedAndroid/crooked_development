...
static void DeDuplicateAbiElementsThread(
  ...
  for (auto it = dump_files_begin; it != dump_files_end; it++) {
    repr::ModuleIR module_ir(exported_headers);
    std::unique_ptr<repr::IRReader> reader =
        repr::IRReader::CreateIRReader(input_format, module_ir);
    if (reader == nullptr) {
    ...
    merger->MergeGraphs(module_ir);
  }
}

std::unique_ptr<linker::ModuleMerger> HeaderAbiLinker::ReadInputDumpFiles() {
  std::unique_ptr<linker::ModuleMerger> merger(new linker::ModuleMerger());
